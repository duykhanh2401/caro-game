<template>
	<div
		class="fromGroup relative mb-2 items-center"
	...
	>
		<label
			v-if="label"
			:class="`${classLabel} ${
				horizontal ? 'flex-0 mr-6 md:w-[100px] w-[60px] break-words mb-0' : ''
		...
		<div class="relative" :class="horizontal ? 'flex-1' : ''">
			<input
				...
				:class="`${classInput} py-[20px] input-control w-full block focus:outline-none h-[40px] ${
			...
			>
				...
				>
					<DKIcon icon="heroicons-outline:eye" v-if="types === 'password'" />
					<DKIcon icon="heroicons-outline:eye-off" v-else />
				</span>

				<span v-if="error" class="text-danger-500">
					<DKIcon icon="heroicons-outline:information-circle" />
				</span>

				<span v-if="validate" class="text-success-500">
					<DKIcon icon="bi:check-lg" />
				</span>
			</div>
		</div>
